...
import { PageHeader, Card, CardHeader, CardBody, Badge, Button, IconButton, Input, Label, Select, Textarea } from "@/components/ui";
import { cn, formatCurrency } from "@/lib/utils";
import {
  ChevronLeft, Plus, Trash2, Save, Send, Download, Calculator,
  Percent, DollarSign, GripVertical, Search, X
} from "lucide-react";

type LineItem = { id: number; desc: string; qty: number; rate: number };

const INITIAL_ITEMS: LineItem[] = [
  { id: 1, desc: "UI/UX design — discovery & wireframes", qty: 24, rate: 120 },
  { id: 2, desc: "Frontend development (Next.js + TypeScript)", qty: 80, rate: 95 },
  { id: 3, desc: "Cloud hosting (annual)", qty: 1, rate: 2400 },
];
...
export default function InvoiceCreatePage() {
  ...
  const [issued, setIssued] = useState("2026-06-28");
  const [due, setDue] = useState("2026-07-28");
  const [currency, setCurrency] = useState("USD");
  const [taxRate, setTaxRate] = useState(8);
  const [discount, setDiscount] = useState(0);
  const [notes, setNotes] = useState("Thank you for your business! Payment is due within 30 days. Late payments subject to 1.5% monthly interest.");
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Build a new invoice with line items, tax, and discounts."
        breadcrumbs={[
          { label: "Apps" },
        ...
        actions={
          <>
            ...
            <Button variant="secondary"><Download className="h-4 w-4" /> Save draft</Button>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="space-y-4">
          {/* Client + dates */}
          <Card>
            <CardHeader title="Invoice details" description="Select a client and set the dates" />
            <CardBody className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                >
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-brand-500 to-accent-500 text-sm font-bold text-white">
              ...
              <div>
                ...
                <Input value="INV-2042" readOnly className="mt-1 bg-gray-50 dark:bg-gray-800/50" />
              </div>
            </CardBody>
          </Card>
          ...
          <Card className="p-0">
            <div className="flex items-center justify-between border-b border-gray-100 p-4 dark:border-gray-800">
              <div>
                ...
                <p className="text-xs text-gray-500">Add the services or products being billed</p>
              </div>
        ...
        <aside className="space-y-4">
          <Card className="sticky top-4">
            ...
            <CardBody className="space-y-3">
              ...
              <div className="space-y-2 border-t border-gray-100 pt-3 dark:border-gray-800">
                <Button className="w-full"><Save className="h-4 w-4" /> Save as draft</Button>
                <Button variant="secondary" className="w-full"><Send className="h-4 w-4" /> Save & send to client</Button>
              </div>
            </CardBody>
          </Card>
          ...
          <Card className="p-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-semibold text-gray-700 dark:text-gray-300">Tip</p>
            </div>
            <p className="mt-1 text-xs text-gray-500">Use the line item rate field to bill hourly, daily, or flat — Nimbus auto-calculates the amount.</p>
          </Card>
        </aside>
      </div>
          ...
          <Card className="w-full max-w-lg p-0" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            ...
            <div className="max-h-80 overflow-y-auto p-2">
              ...
              {filteredClients.length === 0 && (
                <p className="px-4 py-6 text-center text-sm text-gray-500">No clients found</p>
              )}
            </div>
            <div className="border-t border-gray-100 p-3 dark:border-gray-800">
              <Button variant="secondary" className="w-full"><Plus className="h-4 w-4" /> Add new client</Button>
            </div>
          </Card>
        </div>
      )}
